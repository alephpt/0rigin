...
This document describes the architecture of the implemented Python-based Kuramoto model simulation framework with statistical mean field theory (TRD) extensions. The system supports both classical Kuramoto synchronization dynamics and field-theoretic formulations with Hamiltonian dynamics.
...
│       │
│       └── field_theory/            # TRD extensions (NEW)
│           ├── __init__.py
│           ├── hamiltonian/
│           │   ├── __init__.py
...
│           └── TRD_system.py       # High-level TRD system class
│
├── tests/
...
**Status**: Implemented TRD extensions with Hamiltonian dynamics and Klein-Gordon mediator fields.
...
├── pde_solvers/
│   └── base.py                 # PDE solver interface (py-pde integration)
└── TRD_system.py              # TRDSystem high-level interface
```
