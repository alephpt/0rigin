...
TRDCompute::TRDCompute(VkDevice device, VkQueue queue, uint32_t queueFamilyIndex)
    : _device(device)
    , _queue(queue)
...
TRDCompute::~TRDCompute() {
    cleanup();
}

bool TRDCompute::initialize() {
    // Create command pool
...
void TRDCompute::dispatchCompute(VkPipeline pipeline,
...
void TRDCompute::dispatchKuramoto(VkPipeline pipeline,
                                  VkPipelineLayout layout,
                                  VkDescriptorSet descriptorSet,
                                  const void* pushConstants,
                                  size_t pushSize,
...
void TRDCompute::dispatchSyncField(VkPipeline pipeline,
                                   VkPipelineLayout layout,
                                   VkDescriptorSet descriptorSet,
                                   const void* pushConstants,
                                   size_t pushSize,
...
void TRDCompute::dispatchGravityField(VkPipeline pipeline,
                                      VkPipelineLayout layout,
                                      VkDescriptorSet descriptorSet,
                                      const void* pushConstants,
                                      size_t pushSize,
...
void TRDCompute::dispatchDiracEvolution(VkPipeline pipeline,
                                        VkPipelineLayout layout,
                                        VkDescriptorSet descriptorSet,
                                        const void* pushConstants,
                                        size_t pushSize,
...
void TRDCompute::dispatchAccumulation(VkPipeline pipeline,
                                      VkPipelineLayout layout,
                                      VkDescriptorSet descriptorSet,
                                      const void* pushConstants,
                                      size_t pushSize,
...
void TRDCompute::copyBuffer(VkBuffer src, VkBuffer dst, VkDeviceSize size) {
    if (!_isRecording) return;
...
void TRDCompute::fillBuffer(VkBuffer buffer, uint32_t value, VkDeviceSize size) {
    if (!_isRecording) return;
...
void TRDCompute::insertMemoryBarrier() {
    if (!_isRecording) return;
...
void TRDCompute::insertBufferBarrier(VkBuffer buffer,
...
bool TRDCompute::submitBatch(bool waitForCompletion) {
...
void TRDCompute::calculateWorkgroups(uint32_t width, uint32_t height,
