...
MSFTBufferManager::MSFTBufferManager(VkDevice device, VkPhysicalDevice physicalDevice)
    : _device(device), _physicalDevice(physicalDevice) {
}

MSFTBufferManager::~MSFTBufferManager() {
    destroyAllBuffers();
}

std::pair<VkBuffer, VkDeviceMemory> MSFTBufferManager::createBuffer(
    VkDeviceSize size,
...
std::pair<VkBuffer, VkDeviceMemory> MSFTBufferManager::createStorageBuffer(VkDeviceSize size) {
...
VkDeviceMemory MSFTBufferManager::allocateBufferMemory(
    VkBuffer buffer,
    VkMemoryPropertyFlags properties) {
...
uint32_t MSFTBufferManager::findMemoryType(
    uint32_t typeFilter,
    VkMemoryPropertyFlags properties) {
...
void MSFTBufferManager::uploadData(
...
void MSFTBufferManager::downloadData(
    VkDeviceMemory memory,
    void* data,
    VkDeviceSize size) {
...
void* MSFTBufferManager::mapMemory(VkDeviceMemory memory, VkDeviceSize size) {
...
void MSFTBufferManager::unmapMemory(VkDeviceMemory memory) {
    vkUnmapMemory(_device, memory);
}

void MSFTBufferManager::clearBuffer(VkDeviceMemory memory, VkDeviceSize size) {
    void* mappedMemory = mapMemory(memory, size);
...
void MSFTBufferManager::destroyAllBuffers() {
