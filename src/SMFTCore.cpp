...
SMFTCore* SMFTCore::realize() {
    ...
    //report(LOGGER::INFO, "SMFT - Configuring Nova with debug level: %s", _config.debug_level.c_str());
    _actuality = new Nova(_config);
    _actuality->initialized = true;

    // Now that the Nova engine is initialized, we can initialize the SMFT engine.
    _smftEngine = new SMFTEngine(_actuality);
    
    const uint32_t Nx = 128;
    const uint32_t Ny = 128;
    const float Delta = 2.5f;
    const float chiral_angle = 0.0f;
    _smftEngine->initialize(Nx, Ny, Delta, chiral_angle);
    ...
    _smftEngine->setInitialPhases(theta_init);
    _smftEngine->setNaturalFrequencies(omega_init);
...
void SMFTCore::materialize() {
    ...
    std::vector<float> R_field = _essence->_smftEngine->getSyncField();
    float r_sum = 0.0f;
    for (float r : R_field) {
        r_sum += r;
    }
