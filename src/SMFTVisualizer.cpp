#include "SMFTVisualizer.h"
#include <cstring>
#include <algorithm>
#include <iostream>

SMFTVisualizer::SMFTVisualizer(Nova* nova) : _nova(nova), _width(0), _height(0) {
...
SMFTVisualizer::~SMFTVisualizer() {
    destroyResources();
}

void SMFTVisualizer::initialize(uint32_t width, uint32_t height) {
    _width = width;
    _height = height;
...
void SMFTVisualizer::updateRFieldTexture(const std::vector<float>& r_field) {
    if (r_field.size() != _width * _height) {
        std::cerr << "SMFTVisualizer: R field size mismatch!" << std::endl;
        return;
    }
    ...
    if (frame_count++ % 60 == 0) {  // Log every 60 frames
        ...
        avg_r /= r_field.size();
        std::cout << "SMFTVisualizer: Average R = " << avg_r << std::endl;
    }
}

void SMFTVisualizer::mapRToColor(float r, uint8_t* rgba) {
...
void SMFTVisualizer::createTextureResources() {
    // Simplified for now - full implementation would create Vulkan texture
    std::cout << "SMFTVisualizer: Creating " << _width << "x" << _height << " texture resources" << std::endl;
}

void SMFTVisualizer::createStagingBuffer() {
    // Simplified for now - full implementation would create Vulkan staging buffer
    std::cout << "SMFTVisualizer: Creating staging buffer for " << _width << "x" << _height << " texture" << std::endl;
}

void SMFTVisualizer::destroyResources() {
