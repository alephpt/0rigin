#include "TRD.h"
#include "TRDEngine.h" // Assuming this will be created
#include <imgui.h>

TRD* _essence = nullptr;

TRD::TRD() {
    //report(LOGGER::INFO, "TRD - Constructing Nova ..");
...
TRD::~TRD() {
    //report(LOGGER::INFO, "TRD - The End is Nigh ..");
    if (_essence != nullptr) {
        delete _msftEngine;
        delete _actuality;
        _essence = nullptr;
...
TRD* TRD::manifest() {
    //report(LOGGER::INFO, "TRD - Manifesting ..");
    if (_essence == nullptr) {
        TRD* essence = new TRD();
...
TRD* TRD::realize() {
    ...
    //report(LOGGER::INFO, "TRD - Configuring Nova with debug level: %s", _config.debug_level.c_str());
    _actuality = new Nova(_config);
    _actuality->initialized = true;

    // Now that the Nova engine is initialized, we can initialize the TRD engine.
    _msftEngine = new TRDEngine(_actuality);
...
void TRD::materialize() {
    //report(LOGGER::VERBOSE, "TRD - Materializing ..");
...
void TRD::actualize() {
    //report(LOGGER::INFO, "TRD - Actualizing ..");
