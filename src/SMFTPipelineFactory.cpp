...
SMFTPipelineFactory::SMFTPipelineFactory(VkDevice device)
    : _device(device) {
    if (device == VK_NULL_HANDLE) {
        throw std::invalid_argument("SMFTPipelineFactory: Invalid device handle");
    }
}
...
std::vector<uint32_t> SMFTPipelineFactory::loadShaderFile(const std::string& path) {
    ...
    if (!file.is_open()) {
        std::cerr << "SMFTPipelineFactory: Failed to open shader file: " << path << std::endl;
        return {};
    }
    ...
    if (fileSize == 0 || fileSize % sizeof(uint32_t) != 0) {
        std::cerr << "SMFTPipelineFactory: Invalid shader file size: " << fileSize << std::endl;
        return {};
    }
    ...
    if (!buffer.empty() && buffer[0] != 0x07230203) {
        std::cerr << "SMFTPipelineFactory: Invalid SPIR-V magic number in: " << path << std::endl;
        return {};
    }
...
VkShaderModule SMFTPipelineFactory::createShaderModule(const std::vector<uint32_t>& code) {
    if (code.empty()) {
    ...
    if (vkCreateShaderModule(_device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
        std::cerr << "SMFTPipelineFactory: Failed to create shader module" << std::endl;
        return VK_NULL_HANDLE;
    }
...
VkPipeline SMFTPipelineFactory::createComputePipeline(VkShaderModule shaderModule,
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "SMFTPipelineFactory: Failed to create compute pipeline, error: "
                  << result << std::endl;
        return VK_NULL_HANDLE;
    }
...
VkPipeline SMFTPipelineFactory::createPipelineFromShader(const std::string& shaderPath,
                                                         VkPipelineLayout pipelineLayout,
                                                         const std::string& pipelineName) {
    // Load shader bytecode
    auto shaderCode = loadShaderFile(shaderPath);
    if (shaderCode.empty()) {
        std::cerr << "SMFTPipelineFactory: Failed to load shader for "
    ...
    if (shaderModule == VK_NULL_HANDLE) {
        std::cerr << "SMFTPipelineFactory: Failed to create shader module for "
    ...
    if (pipeline == VK_NULL_HANDLE) {
        std::cerr << "SMFTPipelineFactory: Failed to create pipeline for "
                  << pipelineName << std::endl;
    }
...
VkPipeline SMFTPipelineFactory::createKuramotoPipeline(const std::string& shaderPath,
                                                       VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline SMFTPipelineFactory::createSyncFieldPipeline(const std::string& shaderPath,
                                                        VkPipelineLayout pipelineLayout) {
    /**
     ...
     * the effective mass in the SMFT framework (R = √(ℏc/G) * mass density)
     */
    return createPipelineFromShader(shaderPath, pipelineLayout, "SyncField");
}

VkPipeline SMFTPipelineFactory::createGravityFieldPipeline(const std::string& shaderPath,
                                                           VkPipelineLayout pipelineLayout) {
    /**
     ...
     * implementing the SMFT principle that gravity IS sync field geometry
     */
    return createPipelineFromShader(shaderPath, pipelineLayout, "GravityField");
}

VkPipeline SMFTPipelineFactory::createKuramotoStochasticPipeline(const std::string& shaderPath,
                                                                 VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline SMFTPipelineFactory::createDiracStochasticPipeline(const std::string& shaderPath,
                                                              VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline SMFTPipelineFactory::createAccumulationPipeline(const std::string& shaderPath,
                                                           VkPipelineLayout pipelineLayout) {
    /**
