...
/**
 * TRDPipelineFactory - Factory class for creating Vulkan compute pipelines
 *
 * Extracts pipeline creation logic from TRDEngine to follow Single Responsibility Principle.
 ...
 */
class TRDPipelineFactory {
public:
    /**
     * Constructor
     * @param device Vulkan logical device for pipeline creation
     */
    explicit TRDPipelineFactory(VkDevice device);
    ...
    /**
     * Create R-field evolution pipeline with EM coupling
     * Implements: dR/dt = -γ(R - R_kuramoto) + ε·R_source
     * Couples electromagnetic stress-energy to spacetime curvature.
     * @param shaderPath Path to r_field_evolution.comp.spv shader
     * @param pipelineLayout Pre-created pipeline layout
     * @return Created pipeline or VK_NULL_HANDLE on failure
     */
    VkPipeline createRFieldEvolutionPipeline(const std::string& shaderPath,
                                             VkPipelineLayout pipelineLayout);
