...
TRDPipelineFactory::TRDPipelineFactory(VkDevice device)
    : _device(device) {
    if (device == VK_NULL_HANDLE) {
        throw std::invalid_argument("TRDPipelineFactory: Invalid device handle");
    }
}

TRDPipelineFactory::~TRDPipelineFactory() {
    // Clean up any remaining pipelines
    destroyAllPipelines();
}

std::vector<uint32_t> TRDPipelineFactory::loadShaderFile(const std::string& path) {
    // Open shader file in binary mode, positioned at end
    std::ifstream file(path, std::ios::ate | std::ios::binary);

    if (!file.is_open()) {
        std::cerr << "TRDPipelineFactory: Failed to open shader file: " << path << std::endl;
        return {};
    }
    ...
    if (fileSize == 0 || fileSize % sizeof(uint32_t) != 0) {
        std::cerr << "TRDPipelineFactory: Invalid shader file size: " << fileSize << std::endl;
        return {};
    }
    ...
    if (!buffer.empty() && buffer[0] != 0x07230203) {
        std::cerr << "TRDPipelineFactory: Invalid SPIR-V magic number in: " << path << std::endl;
        return {};
    }
...
VkShaderModule TRDPipelineFactory::createShaderModule(const std::vector<uint32_t>& code) {
    if (code.empty()) {
    ...
    if (vkCreateShaderModule(_device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
        std::cerr << "TRDPipelineFactory: Failed to create shader module" << std::endl;
        return VK_NULL_HANDLE;
    }
...
VkPipeline TRDPipelineFactory::createComputePipeline(VkShaderModule shaderModule,
    ...
    if (result != VK_SUCCESS) {
        std::cerr << "TRDPipelineFactory: Failed to create compute pipeline, error: "
                  << result << std::endl;
        return VK_NULL_HANDLE;
    }
...
VkPipeline TRDPipelineFactory::createPipelineFromShader(const std::string& shaderPath,
                                                         VkPipelineLayout pipelineLayout,
                                                         const std::string& pipelineName) {
    // Load shader bytecode
    auto shaderCode = loadShaderFile(shaderPath);
    if (shaderCode.empty()) {
        std::cerr << "TRDPipelineFactory: Failed to load shader for "
    ...
    if (shaderModule == VK_NULL_HANDLE) {
        std::cerr << "TRDPipelineFactory: Failed to create shader module for "
    ...
    if (pipeline == VK_NULL_HANDLE) {
        std::cerr << "TRDPipelineFactory: Failed to create pipeline for "
                  << pipelineName << std::endl;
    }
...
VkPipeline TRDPipelineFactory::createKuramotoPipeline(const std::string& shaderPath,
                                                       VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline TRDPipelineFactory::createSyncFieldPipeline(const std::string& shaderPath,
                                                        VkPipelineLayout pipelineLayout) {
    /**
     ...
     * the effective mass in the TRD framework (R = √(ℏc/G) * mass density)
     */
    return createPipelineFromShader(shaderPath, pipelineLayout, "SyncField");
}

VkPipeline TRDPipelineFactory::createGravityFieldPipeline(const std::string& shaderPath,
                                                           VkPipelineLayout pipelineLayout) {
    /**
     ...
     * implementing the TRD principle that gravity IS sync field geometry
     */
    return createPipelineFromShader(shaderPath, pipelineLayout, "GravityField");
}

VkPipeline TRDPipelineFactory::createKuramotoStochasticPipeline(const std::string& shaderPath,
                                                                 VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline TRDPipelineFactory::createDiracPipeline(const std::string& shaderPath,
                                                    VkPipelineLayout pipelineLayout) {
    /**
     ...
     * - STATUS: DISABLED in TRDEngine::createPipelines()
     *
...
VkPipeline TRDPipelineFactory::createDiracStochasticPipeline(const std::string& shaderPath,
                                                              VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline TRDPipelineFactory::createAccumulationPipeline(const std::string& shaderPath,
                                                           VkPipelineLayout pipelineLayout) {
    /**
...
VkPipeline TRDPipelineFactory::createRFieldEvolutionPipeline(const std::string& shaderPath,
                                                              VkPipelineLayout pipelineLayout) {
    /**
     ...
     * - This creates the EM→gravity coupling in TRD
     *
     * Shader bindings:
...
void TRDPipelineFactory::destroyPipeline(VkPipeline pipeline) {
    if (pipeline != VK_NULL_HANDLE) {
...
void TRDPipelineFactory::destroyAllPipelines() {
