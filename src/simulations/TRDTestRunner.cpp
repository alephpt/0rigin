...
TRDTestRunner::TRDTestRunner(const std::string& config_path)
    ...
    _output_manager = new TRD::OutputManager();
...
TRDTestRunner::TRDTestRunner(const TestConfig& config)
    : _config(config), _config_loaded(true), _nova(nullptr), _nova_initialized(false),
      _engine(nullptr), _engine_initialized(false), _output_manager(nullptr) {
    _output_manager = new TRD::OutputManager();
}

TRDTestRunner::~TRDTestRunner() {
    if (_engine) {
        delete _engine;
    }
    if (_nova) {
...
bool TRDTestRunner::initialize() {
    ...
    std::cout << "✓ TRDTestRunner initialized successfully" << std::endl;
    return true;
}

bool TRDTestRunner::run() {
...
bool TRDTestRunner::runSingleTest(int N) {
    // Set substep ratio
        ...
        if (step % save_every == 0 || step == total_steps - 1) {
            ...
            if (step % (save_every * 10) == 0) {
                std::cout << "  Step " << step << "/" << total_steps
                         << " | R_avg = " << std::setprecision(6) << obs.R_avg
                         << " | norm = " << std::setprecision(6) << obs.norm << std::endl;
            }
        }
    }
...
std::vector<float> TRDTestRunner::initializePhases() const {
...
std::vector<float> TRDTestRunner::initializeOmegas() const {
...
TRDTestRunner::ValidationResult TRDTestRunner::validateNorm(int N) const {
...
TRDTestRunner::ValidationResult TRDTestRunner::validateEnergy(int N) const {
    ValidationResult result;
...
TRDTestRunner::ValidationResult TRDTestRunner::validateConvergence() const {
    ValidationResult result;
...
void TRDTestRunner::saveObservablesToCSV(int N, const std::string& filepath) const {
    auto it = _results.find(N);
    if (it == _results.end()) {
        return;
    }
...
void TRDTestRunner::createOutputDirectories() {
...
std::string TRDTestRunner::getOutputDirectory(int N) const {
    std::ostringstream oss;
    oss << _timestamped_output_dir << "/N_" << N;
    return oss.str();
}

void TRDTestRunner::generateReport(const std::string& output_path) const {
    ...
    report << "===== TRD Test Report =====" << std::endl;
...
void TRDTestRunner::generateVisualizations() const {
...
const std::vector<ObservableComputer::Observables>& TRDTestRunner::getResults(int N) const {
...
bool TRDTestRunner::allTestsPassed() const {
    for (const auto& [N, result] : _validation_results) {
        if (!result.passed()) {
            return false;
        }
