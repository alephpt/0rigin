...
void SMFTCore3D::initialize(const Config& config) {
    ...
    // Log initialization
    std::cout << "[SMFTCore3D] Initialized " << _Nx << "x" << _Ny << "x" << _Nz
              << " grid (" << _N_total << " points)" << std::endl;

    size_t memory_per_field = _N_total * sizeof(float);
    size_t total_memory = 3 * memory_per_field;
    std::cout << "[SMFTCore3D] Memory usage: " << memory_per_field / 1024
              << " KB per field, " << total_memory / 1024 << " KB total" << std::endl;
}

SMFTCore3D::Neighbors3D SMFTCore3D::getNeighbors(uint32_t i, uint32_t j, uint32_t k) const {
...
void SMFTCore3D::initializeUniform(float phase) {
    ...
    std::cout << "[SMFTCore3D] Initialized uniform phase field (θ = "
              << phase << " rad)" << std::endl;
}

void SMFTCore3D::initializeRandom(uint32_t seed) {
    ...
    std::cout << "[SMFTCore3D] Initialized random phase field (seed = "
              << seed << ")" << std::endl;
}

float SMFTCore3D::computeKuramotoCoupling(uint32_t idx) const {
    // Get 3D coordinates
    uint32_t i, j, k;
    coords3D(idx, i, j, k);
...
void SMFTCore3D::evolveKuramotoCPU(float dt) {
...
float SMFTCore3D::getAverageR() const {
...
void SMFTCore3D::enableGPU() {
    if (_device == VK_NULL_HANDLE) {
        std::cerr << "[SMFTCore3D] Error: Cannot enable GPU without valid Vulkan device"
                  << std::endl;
        return;
    }

    _gpu_enabled = true;
    std::cout << "[SMFTCore3D] GPU compute enabled" << std::endl;
