...
class TestFieldTheoryErrors(unittest.TestCase):
    ...
    def test_field_coupling_dimension_mismatch(self):
        ...
        N_oscillators = 10
        grid_shape = (8, 8)  # Different from N

        phi = np.ones(grid_shape)
        pi_phi = np.zeros(grid_shape)
        field = MediatorField(phi=phi, pi_phi=pi_phi)

        # Try to couple with wrong number of oscillators
        with self.assertRaises(ValueError) as context:
            coupling = LocalFieldCoupling(field, coupling_strength=1.0)
            phases = np.ones(N_oscillators)
            # This should fail due to dimension mismatch
            result = coupling.compute_field_coupling(phases, field)
...
class TestVisualizationErrors(unittest.TestCase):
    ...
    def test_plotting_without_matplotlib(self):
        ...
        matplotlib_modules = [m for m in sys.modules if 'matplotlib' in m]
        hidden_modules = {}
        for m in matplotlib_modules:
            hidden_modules[m] = sys.modules[m]
            del sys.modules[m]

        try:
            # Try to use visualization
            from kuramoto.visualization import PhaseSpacePlot

            # Should either import successfully with fallback
            # or raise ImportError with clear message
            try:
                plot = PhaseSpacePlot()
                # If it works, should have some fallback
                self.assertIsNotNone(plot)
            except ImportError as e:
                self.assertIn("matplotlib", str(e).lower())

        finally:
            # Restore matplotlib modules
            for m, mod in hidden_modules.items():
                sys.modules[m] = mod
    ...
    def test_invalid_plot_data(self):
        ...
        # Empty data
        with self.assertRaises(ValueError):
            plot_phase_evolution(times=[], phases=[])
        ...
        with self.assertRaises(ValueError):
            plot_phase_evolution(times=times, phases=phases)
