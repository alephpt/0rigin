...
- Changed `test_smft_gpu.cpp` → `test/test_smft_gpu.cpp`
...
- `test_descriptor_bindings.cpp`: Changed `#include "src/SMFTEngine.h"` → `#include "SMFTEngine.h"`
- `test_smft_gpu.cpp`: Changed `#include "src/SMFTEngine.h"` → `#include "SMFTEngine.h"`

## Verification
All targets now build successfully:
- ✅ `test_descriptor_bindings` - builds and creates executable
- ✅ `test_smft_gpu` - builds and creates executable
...
[ 80%] Built target test_smft_gpu
[ 97%] Built target test_stochastic_particle
[ 97%] Built target SMFT
