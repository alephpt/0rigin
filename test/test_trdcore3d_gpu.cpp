...
/**
 * GPU Integration Test for TRDCore3D
 *
 * Week 2 Tests:
...
class TRDCore3D_GPU {
public:
    TRDCore3D_GPU(VkDevice device, VkPhysicalDevice physicalDevice,
                   VkCommandPool commandPool, VkQueue queue)
        : core_(device, physicalDevice),
          buffer_manager_(std::make_unique<TRDBufferManager>(device, physicalDevice)),
          command_pool_(commandPool),
          queue_(queue),
          device_(device) {}

    void initialize(const TRDCore3D::Config& config) {
        ...
        std::cout << "[TRDCore3D_GPU] Allocated " << 4 * buffer_size / 1024
    ...
    TRDCore3D& getCore() { return core_; }
...
private:
    TRDCore3D core_;
    std::unique_ptr<TRDBufferManager> buffer_manager_;
...
void test_gpu_buffer_allocation() {
    ...
    appInfo.pApplicationName = "TRDCore3D GPU Test";
    ...
    {
        TRDCore3D_GPU gpu_core(device, physicalDevice, commandPool, queue);
...
int main() {
    std::cout << "=== TRDCore3D GPU Integration Test ===" << std::endl;
