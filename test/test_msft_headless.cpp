/**
 * Headless MSFT Physics Test
 * Tests MSFT compute pipeline without graphics/window
...
VkInstance createInstance() {
    ...
    appInfo.pApplicationName = "MSFT Headless Test";
...
int main() {
    std::cout << "=== MSFT Headless Physics Test ===" << std::endl;
    ...
    // 3. Initialize MSFTEngine with test parameters
    MSFTEngine* msftEngine = new MSFTEngine(reinterpret_cast<Nova*>(nova));

    const uint32_t Nx = 128;
    const uint32_t Ny = 128;
    const float Delta = 2.5f;     // Vacuum potential from MSFT.cpp
    const float chiral_angle = 0.0f;

    std::cout << "Initializing MSFT Engine:" << std::endl;
    ...
    msftEngine->initialize(Nx, Ny, Delta, chiral_angle);

    // 4. Set initial conditions (from MSFT.cpp)
    ...
    msftEngine->setInitialPhases(theta_init);
    msftEngine->setNaturalFrequencies(omega_init);

    // 5. Run simulation
    const int numSteps = 1000;
    const int outputInterval = 100;
    const float dt = 0.01f;  // From MSFT.cpp line 89
    const float K = 1.0f;    // From MSFT.cpp line 90 (user adjustable)
    ...
    for (int step = 0; step < numSteps; step++) {
        ...
        if (step % outputInterval == 0 || step == numSteps - 1) {
            std::vector<float> R_field = msftEngine->getSyncField();
    ...
    std::vector<float> R_field = msftEngine->getSyncField();
    std::vector<float> theta_field = msftEngine->getPhaseField();
    std::vector<float> gravity_field = msftEngine->getGravitationalField();
