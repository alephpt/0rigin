...
def compute_wave_velocity_from_phase_gradient(kx, K, damping=0.05):
    """
...
def analyze_wave_surfing(csv_path, config):
    ...
    # Compute particle velocity via finite difference
    dt = time[1] - time[0]
    v_x = np.gradient(pos_x, dt)
    v_y = np.gradient(pos_y, dt)
    ...
    # Correlation between particle velocity and wave propagation direction
    # Wave propagates in +x direction, so correlation is just sign consistency
    correlation = np.corrcoef(v_x[10:], np.ones_like(v_x[10:]))[0, 1]
    ...
    for i, is_locked in enumerate(locked):
        if is_locked:
            current_duration += dt
            lock_duration = max(lock_duration, current_duration)
        else:
            current_duration = 0.0
...
def create_plots(results, config, output_dir):
    ...
    ax1.plot(results['time'], results['v_x'], 'b-', linewidth=2, label='Particle v_x')
    ...
    ax2.axhline(0.15, color='r', linestyle='--', linewidth=2, label='Threshold (15%)')
    ax2.set_xlabel('Time', fontsize=12)
    ax2.set_ylabel('|v_p - v_w| / v_w', fontsize=12)
    ax2.set_title('Velocity Locking Metric', fontsize=14, fontweight='bold')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.set_ylim([0, max(0.5, results['velocity_lock'].max() * 1.1)])
    ...
    ax3.plot(results['time'], wave_front, 'r--', linewidth=2, label='Wave front')
...
def generate_validation_report(results, config, output_dir):
    ...
    # Validation criteria
    velocity_lock_threshold = 0.15
    correlation_threshold = 0.6
    lock_duration_threshold = 5.0
    ...
    overall_pass = velocity_lock_pass and correlation_pass and lock_duration_pass
...
Average velocity: <v_x> = {np.mean(results['v_x']):.6f}
...
2. Velocity-Field Correlation: {'✓ PASS' if correlation_pass else '✗ FAIL'}
   ρ(v_p, ∇R) = {correlation:.4f} (threshold: {correlation_threshold:.2f})
...
Key observations:
- Particle velocity locks to wave velocity within tolerance
- Strong correlation between particle motion and ∇R direction
- Sustained velocity locking over required duration
- This validates the SMFT prediction that particles couple to gradients
  in the synchronization field, leading to emergent "gravity-like" behavior
"""
    else:
        report += """
...
"""
        if not velocity_lock_pass:
            report += "- Velocity locking is too weak (particle not tracking wave)\n"
        if not correlation_pass:
            report += "- Low correlation suggests weak coupling or numerical issues\n"
        if not lock_duration_pass:
            report += "- Lock duration too short (transient effect only)\n"
...
def main():
    ...
    print(f"\n[Particle Dynamics]")
    print(f"  Average velocity: <v_x> = {np.mean(results['v_x']):.6f}")
    print(f"  Velocity-field correlation: ρ = {results['correlation']:.4f}")
